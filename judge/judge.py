...
def ensure_testcase(testcase_id):
    ...
    if not os.path.exists(path):
        ...
        params = {'token': settings.JUDGE_TOKEN}
        while True:
            try:
...
def clone_repo(compiler):
    ...
    if not os.path.exists(repo_root):
        subprocess.check_call(['git', 'clone', '--recursive', compiler.repo_url, repo_root])
...
def checkout_repo(version, target):
    ...
    subprocess.check_call('git archive {} | tar -x -C {}'.format(version.sha, target), cwd=repo_root)
...
def build_docker_image(version, compiler):
    ...
    root_compiler = os.pah.join(root, 'compiler')
    os.mkdir(root_compiler)
    commit_date, comment = checkout_repo(version.sha, root_compiler)
...
def judge_testcase(testcase, exitcode, stdout):
    if testcase['assert'] == 'success_exit':
        return True if exitcode == 0
    if testcase['assert'] == 'failure_exit':
        return True if exitcode != 0
    if testcase['assert'] == 'exitcode':
        return True if exitcode == testcase['exitcode']
...
''')
    ...
    cmd += ['bash', '/testrun/runner.bash', '/compiler/{}.bash'.format(testrun.phase.split()[0]), '/testrun/testcase.txt']
    if 'input' in testcase:
        with open(os.path.join(root, 'input.txt'), 'w') as f:
            f.write(testcase['input'])
        cmd += ['/testrun/input.txt']
    docker_id = subprocess.check_output(cmd)
    is_timeout = True
